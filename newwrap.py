...
class NAFSQuality(Enum):
    STANDARD = "standard"
    PREMIUM = "premium"
    NEURAL = "neural"

@dataclass
class NAFSSynthesisRequest:
    text: str
    nafs_encoding: Optional[bytes] = None
    voice: str = "default"
    language: str = "en-US"
    format: AudioFormat = AudioFormat.MP3
    quality: NAFSQuality = NAFSQuality.STANDARD
    speed: float = 1.0
    pitch: float = 1.0
...
@dataclass
class NAFSSynthesisResponse:
    audio_data: bytes
    format: AudioFormat
    duration: float
    metadata: Dict
    nafs_encoding_used: bytes
    processing_time: float

class SSMLNAFSProcessor:
    """Handles SSML parsing and NAFS phoneme tag processing"""

    def __init__(self, nafs_converter):
        self.nafs_converter = nafs_converter
                ...
                if phoneme_elem.tag.endswith('phoneme'):
                    ...
                    if alphabet == 'nafs':
                        ...
                        try:
                            ...
                            nafs_bytes = bytes.fromhex(ph_value)
                            ipa_text = self.nafs_converter.nafs_to_ipa(nafs_bytes)
...
class NAFSTTSWrapper:
    """
    Production-ready NAFS TTS API wrapper with full SSML support
    """

    def __init__(self,
                 api_key: str,
                 base_url: str = "https://api.nafs-tts.com/v1",
        ...
        self.nafs_converter = NAFSConverter()
        self.ssml_processor = SSMLNAFSProcessor(self.nafs_converter)
        self.stats = {
    ...
    async def synthesize(self, request: NAFSSynthesisRequest) -> NAFSSynthesisResponse:
        """
        ...
        if request.nafs_encoding is None and not request.is_ssml:
            request.nafs_encoding = self._optimize_input(request.text)
    ...
    def create_nafs_ssml(self, text: str, phonetic_mappings: Dict[str, str] = None) -> str:
        ...
        if not phonetic_mappings:
            ...
            for word in words:
                ...
                nafs_bytes = self.nafs_converter.ipa_to_nafs(ipa_text)
                phonetic_mappings[word] = nafs_bytes.hex()
                ...
                if phoneme_elem.tag.endswith('phoneme'):
                    ...
                    if alphabet == 'nafs':
                        ...
                        try:
                            ...
                            self.nafs_converter.nafs_to_ipa(nafs_bytes)
    ...
    async def synthesize_batch(self, requests: List[NAFSSynthesisRequest]) -> List[NAFSSynthesisResponse]:
        """Batch processing with parallel execution"""
        tasks = [self.synthesize(req) for req in requests]
        return await asyncio.gather(*tasks)

    async def stream_synthesis(self, request: NAFSSynthesisRequest) -> AsyncGenerator[bytes, None]:
        ...
        if request.is_ssml:
            ...
            nafs_chunks = self._chunk_nafs_encoding(request.text)

            for chunk in nafs_chunks:
                chunk_request = NAFSSynthesisRequest(
                    text="",
                    nafs_encoding=chunk,
                    voice=request.voice,
    ...
    def _optimize_input(self, text: str) -> bytes:
        ...
        if self._is_ipa_text(text):
            return self.nafs_converter.ipa_to_nafs(text)
        else:
            ipa_text = self._text_to_ipa(text)
            return self.nafs_converter.ipa_to_nafs(ipa_text)
    ...
    def _generate_cache_key(self, request: NAFSSynthesisRequest) -> str:
        """Generate unique cache key for request"""
        key_data = {
            'text': request.text if request.is_ssml else '',
            'nafs_encoding': request.nafs_encoding.hex() if request.nafs_encoding else '',
            'voice': request.voice,
            'language': request.language,
    ...
    async def _process_synthesis(self, request: NAFSSynthesisRequest) -> NAFSSynthesisResponse:
        """Core synthesis processing - integrates with TTS engine"""
        compression_ratio = 2.0
        if request.text and not request.is_ssml:
            compression_ratio = len(request.text.encode()) / len(request.nafs_encoding)

        await asyncio.sleep(0.1)  # Simulated processing delay

        return NAFSSynthesisResponse(
            audio_data=b"simulated_audio_data",
            format=request.format,
            duration=len(request.text) * 0.1 if request.text else 1.0,
            metadata={
                'compression_ratio': compression_ratio,
                'nafs_size': len(request.nafs_encoding) if request.nafs_encoding else 0,
                'original_size': len(request.text.encode()) if request.text else 0,
                'is_ssml': request.is_ssml
            },
            nafs_encoding_used=request.nafs_encoding or b'',
            processing_time=0.1
        )

    def _chunk_nafs_encoding(self, text: str, chunk_size: int = 100) -> List[bytes]:
    ...
    def _update_cache(self, key: str, response: NAFSSynthesisResponse):
...
class PollyNAFSAdapter(NAFSTTSWrapper):
    """Amazon Polly integration adapter with NAFS SSML support"""
    ...
    async def _process_synthesis(self, request: NAFSSynthesisRequest) -> NAFSSynthesisResponse:
        if request.is_ssml:
            ...
            ipa_text = self.nafs_converter.nafs_to_ipa(request.nafs_encoding)
...
class AzureNAFSAdapter(NAFSTTSWrapper):
    """Microsoft Azure TTS integration adapter with NAFS SSML support"""
...
class GoogleNAFSAdapter(NAFSTTSWrapper):
    """Google Cloud TTS integration adapter with NAFS SSML support"""
...
# True NAFSConverter implementation from trueconv.py
import warnings
...
class NAFSConverter:
    ...
    def _create_mappings(self):
        nafs_grid = {
            0x01: '.', 0x02: '\u0306', 0x03: '\u02D0', 0x04: '\u02E5', 0x05: '\u02E6', 0x06: '\u02E7', 
            0x07: '\u02E8', 0x08: '\u02E9', 0x09: '\u02E9\u02E5', 0x0A: '\u02E5\u02E9', 0x0B: '\u02E6\u02E5', 
            0x0C: '\u02E9\u02E8', 0x0D: '\u02E7\u02E6\u02E7', 0x0E: '\u02C8', 0x0F: '\u02CC',
            0x10: '\u03B2', 0x11: 'v', 0x12: '\u00F0', 0x13: 'z', 0x14: '\u0292', 0x15: '\u0290',
            0x16: '\u029D', 0x17: '\u0263', 0x18: '\u0281', 0x19: '\u0295', 0x1A: '\u02A1', 0x1B: '\u0266',
            0x1C: '^h^', 0x1D: '\u0324', 0x1F: '\u02DE',
            0x20: 'b', 0x23: 'd', 0x25: '\u0256', 0x26: '\u025F', 0x27: 'g', 0x28: '\u0262',
            0x2B: '\u0294', 0x2C: '^hh^', 0x2D: '\u0330', 0x2F: '\u0318',
            0x30: '\u0278', 0x31: 'f', 0x32: '\u03B8', 0x33: 's', 0x34: '\u0283', 0x35: '\u0282',
            0x36: '\u00E7', 0x37: 'x', 0x38: '\u03C7', 0x39: '\u0127', 0x3A: '\u02A2', 0x3B: 'h',
            0x3C: '\u0325', 0x3D: '\u032C', 0x3E: '\u033B', 0x3F: '\u0319',
            0x40: 'p', 0x43: 't', 0x45: '\u0288', 0x46: '\u025F', 0x47: 'k', 0x48: 'q',
            0x49: 'i', 0x4A: 'y', 0x4B: '\u0268', 0x4C: '\u0289', 0x4D: '\u026F', 0x4E: 'u',
            0x4F: '\u031D',
            0x50: 'b^h^', 0x53: 'd^h^', 0x55: '\u0256^h^', 0x56: '\u025F^h^', 0x57: 'g^h^',
            0x58: '\u0262^h^', 0x59: 'e', 0x5A: '\u00F8', 0x5B: '\u0258', 0x5C: '\u0275', 0x5D: '\u0264',
            0x5E: 'o', 0x5F: '\u031E',
            0x60: 'p^h^', 0x63: 't^h^', 0x65: '\u0288^h^', 0x66: '\u025F^h^', 0x67: 'k^h^',
            0x68: 'q^h^', 0x69: '\u025B', 0x6A: '\u0153', 0x6B: '\u025C', 0x6C: '\u025E', 0x6D: '\u028C',
            0x6E: '\u0254', 0x6F: '\u031F',
            0x70: 'b\u031A', 0x73: 'd\u031A', 0x75: '\u0256\u031A', 0x76: '\u025F\u031A', 0x77: 'g\u031A', 0x78: '\u0262\u031A',
            0x79: 'a', 0x7A: '\u0276', 0x7B: '\u00E4', 0x7C: '\u0252\u0308', 0x7D: '\u0251', 0x7E: '\u0252',
            0x7F: '\u031F',
            0x80: 'p\u031A', 0x83: 't\u031A', 0x85: '\u0288\u031A', 0x86: '\u025F\u031A', 0x87: 'k\u031A', 0x88: 'q\u031A',
            0x89: '\u026A', 0x8A: '\u028F', 0x8B: '\u00E6', 0x8C: '\u0250', 0x8D: '\u028A', 0x8E: '\u0259',
            0x8F: '\u02DE',
            0x90: 'm', 0x91: '\u0271', 0x93: 'n', 0x95: '\u0273', 0x96: '\u0272', 0x97: '\u014B',
            0x98: '\u0274', 0x99: '^\u0303^', 0x9F: '\u0308',
            0xA0: 'w', 0xA1: '\u028B', 0xA3: '\u0279', 0xA5: '\u027B', 0xA6: 'j', 0xA7: '\u0270',
            0xAE: '\u2198',
            0xB0: '\u028D', 0xB1: '\u0265', 0xB2: '\u026C', 0xB3: 'l', 0xB4: '\u026E', 0xB5: '\u026D',
            0xB6: '\u028E', 0xB7: '\u029F', 0xBE: '\u2197',
            0xC0: '\u0298', 0xC2: '\u01C0', 0xC3: '\u01C1', 0xC4: '\u01C3', 0xC6: '\u01C2', 0xCF: '|',
            0xD0: '\u0253', 0xD2: '\u0257', 0xD3: '\u0255', 0xD4: '\u0291', 0xD6: '\u0284', 0xD7: '\u0260',
            0xD8: '\u029B', 0xDF: '\u2016',
            0xE0: '\u0299', 0xE1: '\u2C71', 0xE2: '\u027E', 0xE3: 'r', 0xE4: '\u027A', 0xE5: '\u027D',
            0xE6: '\u0267', 0xE8: '\u0280',
            0xF0: '^w^', 0xF1: '\u033C', 0xF2: '\u032A', 0xF3: '\u033A', 0xF4: '^jjj^', 0xF5: '^jj^',
            0xF6: '^j^', 0xF7: '^\u0263^', 0xF8: '\u02E4\u02E4', 0xF9: '\u02E4', 0xFB: '^\u02C0^',
            0xFC: '\u02DE', 0xFD: '\u02BC', 0xFE: '\u031A',
        }
        ipa_to_nafs = {}
        nafs_to_ipa = {}
        ...
        affricate_mappings = {
            't\u0283': [0x43, 0x34], 'd\u0292': [0x23, 0x14], 'ts': [0x43, 0x33], 'dz': [0x23, 0x13],
            't\u0255': [0x43, 0xD3], 'd\u0291': [0x23, 0xD4], 't\u0282': [0x43, 0x35], 'd\u0290': [0x23, 0x15],
        }
        for affricate, nafs_bytes in affricate_mappings.items():
            ipa_to_nafs[affricate] = nafs_bytes
    ...
    def nafs_to_ipa(self, nafs_bytes: bytes) -> str:
        result = []
        i = 0
        while i < len(nafs_bytes):
            if i + 1 < len(nafs_bytes):
                two_byte_key = (nafs_bytes[i], nafs_bytes[i+1])
                if two_byte_key in self.nafs_to_affricate_dict:
                    result.append(self.nafs_to_affricate_dict[two_byte_key])
                    i += 2
                    continue
            byte_val = nafs_bytes[i]
    ...
    def is_lossless(self, ipa_string: str) -> bool:
        try:
            nafs_bytes = self.ipa_to_nafs(ipa_string)
            reconstructed = self.nafs_to_ipa(nafs_bytes)
...
# Utility functions
def create_nafs_adapter(provider: str, api_key: str) -> NAFSTTSWrapper:
    """Factory function to create appropriate adapter"""
    adapters = {
        'polly': PollyNAFSAdapter,
        'azure': AzureNAFSAdapter,
        'google': GoogleNAFSAdapter,
        'default': NAFSTTSWrapper
    }

    adapter_class = adapters.get(provider.lower(), NAFSTTSWrapper)
    return adapter_class(api_key=api_key)

# Example usage with SSML
async def example_ssml_usage():
    """Example of NAFS SSML integration"""

    nafs_tts = create_nafs_adapter('polly', 'your-api-key')

    # Example 1: Create SSML with NAFS phoneme tags
    regular_text = "Hello world"
    ssml_with_nafs = nafs_tts.create_nafs_ssml(regular_text)
    print("Generated SSML with NAFS:")
    print(ssml_with_nafs)

    # Example 2: Process existing SSML with NAFS tags
    ssml_request = NAFSSynthesisRequest(
        text=ssml_with_nafs,
        voice="neural-emma",
        language="en-US",
        format=AudioFormat.MP3,
        quality=NAFSQuality.NEURAL
    )

    response = await nafs_tts.synthesize(ssml_request)
    print(f"SSML synthesis completed in {response.processing_time}s")

    # Example 3: Validate NAFS SSML
    validation = nafs_tts.validate_nafs_ssml(ssml_with_nafs)
    print(f"SSML validation: {validation}")

    # Get enhanced statistics
    stats = nafs_tts.get_statistics()
    print(f"Enhanced stats: {stats}")

if __name__ == "__main__":
    asyncio.run(example_ssml_usage())
